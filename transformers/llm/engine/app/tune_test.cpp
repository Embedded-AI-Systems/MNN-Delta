...
static MNNForwardType backend_type_convert(const std::string& type_str) {
    ...
    if (type_str == "hiai") return MNN_FORWARD_USER_0;
...
float profileLLM(Llm* llm, int prefill_len) {
    ...
    auto logits = llm->forward(test_prompt, true);
    logits->readMap<float>();
    auto et = std::chrono::system_clock::now();
    float timeCost = std::chrono::duration_cast<std::chrono::microseconds>(et - st).count() / 1000.f;
    llm->trace(false);
    llm->reset();   
...
// void tune() {
    ...
    std::vector<int> prefill_len_list = {50, 100, 200, 300, 400, 500, 600, 800, 1000, 1200, 1500};
    ...
    // float qkv_sim_hiai = inf;
    // float up_proj_sim_hiai = inf;
    // float down_proj_sim_hiai = inf;
    // float prefill_llm_sim_hiai = inf;
    // auto hiai_exe = init_tune(backend_type_convert("hiai"), 4);
    // if (hiai_exe != nullptr) {
    //     for (auto prefill_len : prefill_len_list) {
    //         MNN::Express::ExecutorScope scope(hiai_exe);
    //         // qkv_sim_hiai = profileMatMul(prefill_len, hidden_size, hidden_size);
    //         // up_proj_sim_hiai = profileMatMul(prefill_len, hidden_size, intermediate_size);
    //         // down_proj_sim_hiai = profileMatMul(prefill_len, intermediate_size, hidden_size);
    //         prefill_llm_sim_hiai = profileLLM(llm, prefill_len);
    //     }
    // }
}
